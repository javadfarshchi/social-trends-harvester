...
async def get_trending(
    ...
    region: str = Query(
        default=settings.DEFAULT_REGION, description="Region code (ISO 3166-1 alpha-2)"
    ),
    count: int = Query(
        default=settings.DEFAULT_COUNT,
        description=f"Number of items to return (1-{settings.MAX_COUNT})",
    ),
    ...
    try:
        ...
        if count < 1 or count > settings.MAX_COUNT:
            raise HTTPException(
                status_code=400, detail=f"Count must be between 1 and {settings.MAX_COUNT}"
            )
